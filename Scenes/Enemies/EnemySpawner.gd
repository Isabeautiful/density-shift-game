...
@export var min_player_distance: float = 15.0  # Nova variável
@export var max_player_distance: float = 30.0  # Nova variável
@export var enabled: bool = true
@export var enemy_lifetime: float = 60.0
@export var min_distance_between_enemies: float = 8.0  # Distância mínima entre inimigos
...
func spawn_enemy():
    ...
    var max_attempts = 8  # Aumentado para mais tentativas
    ...
    while not valid_position and spawn_attempts < max_attempts:
        ...
        # Verifica distância de outros inimigos
        var too_close = false
    ...
    # Fallback: se não encontrou posição válida, spawn em posição aleatória
    ...
    # Adiciona uma rotação inicial aleatória
    if enemy.has_method("rotate_y"):
        enemy.rotate_y(randf_range(0, 2 * PI))
